...
# Select ROD (recent-onset depression) data for further analysis
rod_fat_sim <- pronia_sim %>%
  filter(Studygroup %in% 'ROD')
...
# Adjusted R² for linear model, quadratic, and gamma model
...
predict_chr_fat_all <- chr_fat_sim %>%
  dplyr::select(PSN)
predict_rod_fat_all <- rod_fat_sim %>%
  dplyr::select(PSN)
...
for (i in 1:length(rois_average_fat)){
  ...
  # Apply the model to the ROD group
  rod2predict <- rod_fat_sim %>%
    dplyr::select(rois_average_fat[i], age, sex)%>% # first select the vars of interest
    rename(independent_var = rois_average_fat[i], # rename them that they match the HC-model names
           age_var = age, 
           sex_var = sex)%>%
    ...
    dplyr::select(-c(age_var, sex_var)) 
  # remove the variables from the data-frame that are not needed
  colnames(rod2predict) <- c(rois_average_fat[i], paste0(rois_average_fat[i], '_predicted'),
                             paste0(rois_average_fat[i], '_dev'))
  predict_rod_fat_all <- predict_rod_fat_all %>% # combine the different rois
    cbind(rod2predict)
...
# Combine the predicted FA-t values and deviations for CHR group
rod_final_fat <- predict_rod_fat_all %>%
  dplyr::select(c(PSN, ends_with('predicted'), ends_with('dev')))
...
tbss_final_fat %>%
  filter(Studygroup %in% c('CHR', 'HC', 'ROP', 'ROD'))%>%
  ...
  scale_x_discrete(limits = c('HC','CHR','ROP', 'ROD'),
...
# Filter the data to include only the four groups: 'CHR', 'HC', 'ROP', and 'ROD'
tbss_final_fat %>%
  filter(Studygroup %in% c('CHR', 'HC', 'ROP', 'ROD'))%>%
...
write.csv(rop_chr_vars, 'simulated_data/cca_vars_rop_chr_for_cv.csv', row.names = FALSE)
...
```{r permute_cca, echo = TRUE, warning = FALSE, messages = FALSE, results = TRUE}
...
```{r apply to ROD}
# analysis:
rod_vars <- tbss_final_fat %>%
  filter(Studygroup %in% c('ROD'))%>%
  mutate(sex_num = case_when(sex == 'male' ~ 0,
                             sex == 'female' ~ 1))%>%
  # first change the variables so that always the higher the better:
  mutate(sips_p_t0_re = -sips_p_t0,
         sips_n_t0_re = -sips_n_t0,
         sips_g_t0_re = -sips_g_t0,
         sips_d_t0_re = -sips_d_t0,
  ...
  mutate(across(everything(.), ~ as.character(.)))%>%
  filter(!sum(is.na(c_across())) == ncol(.) - 2)%>%
  mutate(studygroup_num = case_when(Studygroup %in% 'ROD' ~ 0.5))%>%
  ungroup()%>%
  dplyr::select(-Studygroup)
# impute the missing values
### impute for ROD based on ROP+CHR
# Get group-wise means and median for imputation
impute_means <- rop_chr_vars %>%
  mutate(across(-c(PSN), ~ as.numeric(.))) %>%
  summarise(across(-(c(uhr_1stdegree_re, PSN, studygroup_num)), mean, na.rm = TRUE))

get_mode <- function(x) {
  ux <- na.omit(unique(x))
  ux[which.max(tabulate(match(x, ux)))]
}
impute_mod <- rop_chr_vars %>%
  summarise(mod = get_mode(uhr_1stdegree_re)) %>%
  pull(mod)

rod_vars_imputed <- rod_vars %>%
  mutate(across(-c(PSN, uhr_1stdegree_re, studygroup_num), ~ ifelse(is.na(.), impute_means[[cur_column()]], .))) %>%
  mutate(uhr_1stdegree_re = ifelse(is.na(uhr_1stdegree_re), impute_mod, uhr_1stdegree_re))

cca_vars_rod <- rod_vars_imputed %>%
  column_to_rownames('PSN')%>%
  # dplyr::select(-studygroup_num)%>%
  mutate(across(everything(), ~ as.numeric(.)))

# prepare dti:
rod_dti <- tbss_final_fat %>%
  filter(Studygroup %in% c('ROD'))%>%
  dplyr::select(PSN, Studygroup, significant_rois_names_3groups$roi_names)%>%
  filter(PSN %in% rod_vars$PSN)
cca_dti_rod <- rod_dti %>% 
  dplyr::select(-c(Studygroup, contains('Average')))%>%
  column_to_rownames('PSN')
...
# ---------------------------------------------------------------------------- #
# Apply the CCA to the ROD group.
# ---------------------------------------------------------------------------- #
# create this with perm_winkler:

perm_projected_test_set <- function(Y_train, X_train, Y_test, X_test, nP = 1000, Pset_test = NULL) {
  # Y_train: original data from training matrix 1 (here vars)
  # X_train: original data from training matrix 2 (here dwi)
  # Y_test: external data from test matrix 1 (here vars, ROD)
  # X_test: external data from test matrix 2 (here dwi, ROD)
  
  # Step 1: Fit CCA on training set using permcca_winkler (but we just need canonical weights)
  ...
  # Step 2: Center test data using training means
  Y_test_centered <- sweep(Y_test, 2, colMeans(Y_train), "-")
  X_test_centered <- sweep(X_test, 2, colMeans(X_train), "-")
  
  # Step 3: Project test data
  Y_test_proj <- Y_test_centered %*% A
  X_test_proj <- X_test_centered %*% B
  
  true_cc <- diag(cor(Y_test_proj, X_test_proj))
  
  # Step 4: Permute and compute null distribution
  set.seed(123)
  n_test <- nrow(Y_test_centered)

  K <- length(true_cc)
  perm_cc <- matrix(NA, nrow = nP, ncol = K)
  
  for (p in 1:nP) {
    if (is.null(Pset_test)) {
      idx <- sample(n_test)
    } else {
      idx <- as.matrix(Pset_test[, p, drop = FALSE])
    }
    
    Y_perm <- Y_test_centered[idx, ]
    Y_perm_proj <- Y_perm %*% A
    
    perm_cc[p, ] <- diag(cor(Y_perm_proj, X_test_proj))
  }
  
  # Step 5: Empirical p-values
  # Uncorrected p-values
  p_uncorrected <- colMeans(sweep(perm_cc, 2, true_cc, FUN = ">="))
  
  p_fwer_stepdown <- cummax(p_uncorrected)

  return(list(
  canonical_correlations_test = true_cc,
  permuted_distributions = perm_cc,
  p_values_uncorrected = p_uncorrected,
  p_values_fwer_stepdown = p_fwer_stepdown,
  A = A,
  B = B
  ))
}
rod_result <- perm_projected_test_set(as.matrix(cca_vars_rop_chr), as.matrix(cca_dti_rop_chr),
                                      as.matrix(cca_vars_rod), as.matrix(cca_dti_rod))
#rod_result_final <- data.frame(rod_result$p_values,
#                               rod_result$canonical_correlations_test)
rod_result_final <- data.frame(rod_result$p_values_fwer_stepdown,
                               rod_result$p_values_uncorrected,
                               rod_result$canonical_correlations_test)
print(rod_result_final)
```
