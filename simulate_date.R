...
# Set up the number of individuals per group
n_HC <- 298
n_CHR <- 183
n_ROP <- 206
...
sim_data <- data.frame(
  ...
  Studygroup = factor(rep(c("HC", "CHR", "ROP"), times = c(n_HC, n_CHR, n_ROP)))
)
...
fa_data <- data.frame(
  ...
  ROI = rep(rois, times = n_HC + n_CHR + n_ROP),
  FA_value = NA
)
...
sim_data$wais_v <- c(
  rnorm(n_HC, mean = 50, sd = 10),
  rnorm(n_CHR, mean = 47, sd = 10),
  rnorm(n_ROP, mean = 45, sd = 12)
)
sim_data$wais_mr <- c(
  rnorm(n_HC, mean = 55, sd = 11),
  rnorm(n_CHR, mean = 52, sd = 11),
  rnorm(n_ROP, mean = 50, sd = 13)
)
...
# WAIS vocabulary (92% overall, 91% ROP, 94% CHR)
...
# WAIS matrices (90% overall, 91% ROP, 90% CHR)
