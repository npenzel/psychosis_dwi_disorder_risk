...
run_cv_aligned <- function(X_mat,
  ...
  for (rep_idx in seq_len(n_repeats)) {
    ...
    for (fold_idx in seq_len(n_folds)) {
      ...
      # here i could apply the winkler logic!
...
# test only the 2 first components
...
loglik_obs <- -rev(cumsum(rev(log(1 - cc_train_mat_mean[1:K]^2))))
...
stepdown_results <- data.frame(
  Component = paste0("Comp ", seq_len(length(cc_train_mat_mean[1:K]))),
  Observed_CC = round(cc_train_mat_mean[1:K],3),
