...
for (repeat_idx in 1:n_repeats) {
  ...
  for (fold_idx in 1:n_folds) {
    ...
    perms_training <- create_permutation(X_train %>% as.data.frame(.), 1000)
    perms_test <- create_permutation(X_test %>% as.data.frame(.), 1000)
...
saveRDS(output, 'simulated_data/CV_output.Rds')
